...
class FashionIQDataset(Dataset):
    ...
    def _find_image_path(self, image_name: str):
        """
        Find image in dress_type subfolders (dress/, shirt/, toptee/)
        Supports both .jpg and .png extensions
        """
        # Try each dress_type folder
        for dress_type in self.dress_types:
            for ext in ['.jpg', '.png', '.jpeg']:
                img_path = self.dataset_path / dress_type / f"{image_name}{ext}"
                if img_path.exists():
                    return img_path
        
        # If not found, raise error with helpful message
        raise FileNotFoundError(
            f"Image '{image_name}' not found in any dress_type folder: {self.dress_types}"
        )
        ...
        try:
            if self.mode == 'relative':
                ...
                if self.split in ['train', 'val']:
                    # Find reference image in dress_type folders
                    reference_image_path = self._find_image_path(reference_name)
                    reference_image = self.preprocess(PIL.Image.open(reference_image_path))
                    target_name = self.triplets[index]['target']
                    # Find target image in dress_type folders
                    target_image_path = self._find_image_path(target_name)
                ...
                # Find image in dress_type folders
            ...
            print(f"Warning: Skipping sample {index} due to error: {e}")
            return None
