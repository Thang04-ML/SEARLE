...
def fiq_generate_val_predictions(clip_model: CLIP, relative_val_dataset: Dataset, ref_names_list: List[str],
    ...
    for batch in tqdm(relative_val_loader):
        ...
        for i, ref in enumerate(reference_names):
            try:
                idx = ref_names_list.index(ref)
                batch_tokens_list.append(pseudo_tokens[idx].unsqueeze(0))
        ...
        batch_tokens = torch.vstack(batch_tokens_list)
        
        # Filter lists/captions based on valid_indices
        input_captions = [input_captions[i] for i in valid_indices]
        # flattened_captions were for input_captions construction, we don't need them anymore
        # but we need input_captions_reversed
        
        # Re-construct reversed captions for valid items
        # flattened_captions contains [cap1a, cap1b, cap2a, cap2b, ...]
        # valid_indices map to the query index (pair of captions)
        
        valid_input_captions_reversed = []
        for i in valid_indices:
             # Logic from original code:
             # flattened_captions[i*2 + 1] and flattened_captions[i*2]
             cap1 = flattened_captions[i*2].strip('.?, ')
             cap2 = flattened_captions[i*2 + 1].strip('.?, ')
             valid_input_captions_reversed.append(f"a photo of $ that {cap2} and {cap1}")
        ...
        tokenized_input_captions_reversed = clip.tokenize(valid_input_captions_reversed, context_length=77).to(device)
        ...
        target_names_list.extend([target_names[i] for i in valid_indices])
...
def fiq_compute_val_metrics(relative_val_dataset: Dataset, clip_model: CLIP, index_features: torch.Tensor,
    ...
    # Pre-compute precisions for all queries
    # labels shape: (num_queries, num_db_images)
    # cumsum along dim 1 gives the count of correct items up to each position
    precisions = torch.cumsum(labels, dim=1) / torch.arange(1, labels.shape[1] + 1).to(device)
    
    # We only care about precision at positions where the label is 1 (correct retrieval)
    # But since there is only 1 ground truth per query in FIQ (implied by logic above),
    # AP is simply the precision at the rank of the correct item.
    # However, for general mAP calculation (and consistency if multiple GTs ever supported):
    for i in range(len(labels)):
        # Get ranks where relevant items are found
        relevant_ranks = torch.where(labels[i])[0] # 0-indexed ranks
        if len(relevant_ranks) == 0:
            ap_at10.append(0)
            ap_at50.append(0)
            continue
            
        # Select precisions at relevant ranks
        relevant_precisions = precisions[i][relevant_ranks]
        
        # Filter for top K
        relevant_ranks_at10 = relevant_ranks[relevant_ranks < 10]
        if len(relevant_ranks_at10) > 0:
             ap_at10.append(torch.mean(precisions[i][relevant_ranks_at10]).item())
        else:
             ap_at10.append(0)

        relevant_ranks_at50 = relevant_ranks[relevant_ranks < 50]
        if len(relevant_ranks_at50) > 0:
             ap_at50.append(torch.mean(precisions[i][relevant_ranks_at50]).item())
        else:
             ap_at50.append(0)

    return {'fiq_recall_at10': recall_at10,
            'fiq_recall_at50': recall_at50,
            'fiq_map_at10': np.mean(ap_at10) * 100,
            'fiq_map_at50': np.mean(ap_at50) * 100}
...
def main():
    ...
    if args.dataset.lower() == 'fashioniq':
        ...
        for dress_type in ['shirt', 'dress', 'toptee']:
            ...
            maps_at10.append(fiq_metrics['fiq_map_at10'])
            maps_at50.append(fiq_metrics['fiq_map_at50'])

            # Print mAP metrics as well
            ...
            results_list.append({
                'Category': dress_type,
                'Recall@10': fiq_metrics['fiq_recall_at10'],
                'Recall@50': fiq_metrics['fiq_recall_at50'],
                'mAP@10': fiq_metrics['fiq_map_at10'],
                'mAP@50': fiq_metrics['fiq_map_at50']
            })

        print(f"average_fiq_recall_at10 = {np.mean(recalls_at10):.2f}")
        print(f"average_fiq_recall_at50 = {np.mean(recalls_at50):.2f}")
        print(f"average_fiq_map_at10 = {np.mean(maps_at10):.2f}")
        print(f"average_fiq_map_at50 = {np.mean(maps_at50):.2f}")
        
        results_list.append({
            'Category': 'Average',
            'Recall@10': np.mean(recalls_at10),
            'Recall@50': np.mean(recalls_at50),
            'mAP@10': np.mean(maps_at10),
            'mAP@50': np.mean(maps_at50)
        })
        
        df = pd.DataFrame(results_list)
        csv_name = f'fashioniq_results_{args.eval_type}.csv'
